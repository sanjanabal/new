...
show_menu
    ...
    if [[ $opt = "" ]]; then 
		...
		hadoop jar task.jar task1a /user/hive/warehouse/h1b_final /niit/task1a
		hadoop fs -cat /niit/task1a/p*
		...
		read var
		hive -e "select worksite,count(*) as total from h1b_final where LOWER(job_title) like '%data engineer%' and year=$var group by worksite order by total desc limit 5;"
        show_menu;
        ;;
	...
	5) clear;
	    ...
	    hadoop jar task.jar task3 /user/hive/warehouse/h1b_final /niit/task3
	    hadoop fs -cat /niit/task3/p*
       show_menu;
        ;;
	    ...
	    hive -e "select job_title,count(*) as total from h1b_final where LOWER(case_status) like 'certified' and year=$var group by job_title order by total desc limit 10;"
        show_menu;
        ;;
	...
	11) clear;
	    ...
	    case $n in
                1)	echo "Full time"
                    pig -x local task8a.pig
                    ;;		
                    
                2) 	echo "Part time"
                   pig -x local task8b.pig
                    ;;
	...
	option_picked "10) Which are the top 10 job positions which have the  success rate more than 70% in petitions and total petitions filed more than 1000?"
		pig -x local task10.pig
        show_menu;
        ;;

	14) clear;
	option_picked "11) Export result for question no 10 to MySql database."
		mysql -u root -p'sanjana' -e 'drop database question11;create database if not exists question11;use question11;create table question11(job_title varchar(100),success_rate float,petitions int);';

		sqoop export --connect jdbc:mysql://localhost/question11 --username root --password 'sanjana' --table question11 --update-mode allowinsert  --export-dir /niit/question11 --input-fields-terminated-by '\t';

		echo -e '\n\nContents Exported to MySQL Database:\n\n'

		mysql -u root -p'sanjana' -e 'select * from question11.question11';
        show_menu;
        ;;
